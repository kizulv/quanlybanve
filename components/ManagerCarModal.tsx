...
import { Save, LayoutGrid, Settings2, Info, CheckCircle2, X, AlertCircle } from 'lucide-react';
            ...
            {type === BusType.CABIN ? (
              ...
              <div className="flex gap-8">
                {[...Array(config.cols)].map((_, c) => (
                  <div key={c} className="bg-white rounded-2xl shadow-sm border border-slate-300 w-[160px] relative overflow-hidden">
                    {/* Header */}
                    <div className="bg-slate-50 border-b border-slate-100 py-3 text-center">
                      <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Dãy {String.fromCharCode(65+c)}</span>
                    </div>
              ...
              <div className="flex gap-6 md:gap-10">
                {[...Array(config.floors)].map((_, floorIndex) => {
                   ...
                   return (
                    <div key={floor} className="relative bg-white rounded-[2.5rem] shadow-sm border border-slate-300 p-5 min-w-[200px]">
                       {/* Floor Label */}
