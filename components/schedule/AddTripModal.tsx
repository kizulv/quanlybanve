...
export const AddTripModal: React.FC<AddTripModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        let rId = initialData.routeId
          ? String(initialData.routeId)
          : routes.find((r) => r.name === initialData.route)?.id || "";
  ...
  const conflictWarning = useMemo(() => {
    ...
    const otherRouteTrip = tripsOnDay.find(
      (t) =>
        ((t.routeId && String(t.routeId) !== String(selectedRouteId)) ||
          t.route !== route?.name) &&
        t.licensePlate === selectedBus.plate,
    );
    if (otherRouteTrip)
      return {
        title: "Trùng xe với tuyến khác",
        message: `Xe ${selectedBus.plate} đã được xếp cho tuyến "${otherRouteTrip.route}".`,
      };
    return null;
  }, [
    selectedRouteId,
  ...
  const handleSubmit = async () => {
    ...
    if (bus.layoutConfig) {
      ...
      if (bus && bus.layoutConfig) {
        newLayoutSeats = generateSeatsFromLayout(
          bus.layoutConfig,
          price,
          bus.type,
        );
