...
export const AddTripModal: React.FC<AddTripModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (bus.layoutConfig) {
      ...
      // Floor seats: Cabin 6 sàn, Giường 12 sàn
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-5">
          ...
          <div><label className="block text-sm font-medium text-slate-700 mb-2">Chọn xe vận hành <span className="text-red-500">*</span></label>
            {!selectedRouteId ? <div className="bg-slate-100 border border-slate-200 border-dashed rounded-lg p-6 flex flex-col items-center justify-center text-slate-400"><BusFront size={24} className="mb-2 opacity-50" /><span className="text-sm">Vui lòng chọn tuyến đường trước</span></div>
        ...
        <div className="space-y-5">
          <div className="bg-white p-4 rounded-xl border border-slate-200 h-full flex flex-col shadow-sm">
            ...
            <div className="space-y-5 flex-1">
              ...
              <div className="mt-auto pt-4 border-t border-slate-100"><div className="flex items-start gap-2 text-[10px] text-slate-500 bg-slate-50 p-2 rounded border border-slate-100"><Info size={14} className="shrink-0 mt-0.5 text-slate-400" /><p>Hệ thống tự động cấu hình 6 vé sàn cho xe phòng hoặc 12 vé sàn cho xe giường đơn.</p></div></div>
            </div>
          </div>
        </div>
      </div>
