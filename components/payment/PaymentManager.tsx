  ...
  const allGroupedPayments = useMemo(() => {
    ...
    payments.forEach((payment) => {
      ...
      if (typeof booking === "string") {
        ...
        bKey = booking.id || booking._id || "orphaned";
      }

      // Look up full booking details from the bookings list
      const fullBooking = bookings.find((b) => b.id === bKey);
      const passengerName =
        booking?.passenger?.name ||
        fullBooking?.passenger?.name ||
        "Khách lẻ / Đã xóa";
      const passengerPhone =
        booking?.passenger?.phone || fullBooking?.passenger?.phone || "N/A";
      ...
      groups[bKey].totalCollected += Number(payment.totalAmount) || 0;
