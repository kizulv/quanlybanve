...
export const SeatDetailModal: React.FC<SeatDetailModalProps> = ({
    ...
    if (isOpen) {
        if (booking && seat) {
          ...
          if (seat.status === SeatStatus.BOOKED) {
            ...
            setForm({ phone: "", pickup: "", dropoff: "", note: seat.note || "", name: "" });
        }
  ...
  const handleMoneyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setPaymentInput(prev => ({ ...prev, [name]: parseInt(value.replace(/\D/g, "") || "0", 10) }));
  };

  const handleSaveOnly = async () => {
      setIsSaving(true);
      try { await onSave({ ...form, phone: form.phone.replace(/\D/g, ""), pickupPoint: form.pickup, dropoffPoint: form.dropoff }); } finally { setIsSaving(false); }
  };

  const handlePaySeat = async () => {
      if ((paymentInput.paidCash + paymentInput.paidTransfer) === 0) return;
      setIsSaving(true);
      try { await onSave({ ...form, phone: form.phone.replace(/\D/g, ""), pickupPoint: form.pickup, dropoffPoint: form.dropoff }, { action: 'PAY', payment: paymentInput }); } finally { setIsSaving(false); }
  };

  const handleRefundSeat = async () => {
      setIsSaving(true);
      try { await onSave({ ...form, phone: form.phone.replace(/\D/g, ""), pickupPoint: form.pickup, dropoffPoint: form.dropoff }, { action: 'REFUND' }); } finally { setIsSaving(false); }
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 border-r border-indigo-900/50 bg-indigo-950 space-y-4">
            <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-1.5">
                        <div className={`w-1.5 h-1.5 rounded-full ${isSold ? 'bg-green-400 animate-pulse' : isBooked ? 'bg-yellow-400' : 'bg-indigo-400'}`}></div>
                        <p className="text-[10px] text-indigo-400 font-bold uppercase tracking-wider">
                            {isSold ? 'Đã thanh toán' : isBooked ? 'Đã đặt chỗ' : 'Đang giữ'}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3 pt-2">
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Họ tên khách hàng</label>
                    <div className="relative group">
                        <div className="absolute left-2.5 top-2 pointer-events-none text-indigo-500 group-focus-within:text-yellow-400 transition-colors">
                            <User size={14} />
                        </div>
                        <input 
                ...
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Số điện thoại</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Điểm đón</label>
                        <div className="relative">
                            <div className="absolute left-2.5 top-2 pointer-events-none text-indigo-500">
                                <MapPin size={14} />
                            </div>
                    ...
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Điểm trả</label>
                        <div className="relative">
                            <div className="absolute left-2.5 top-2 pointer-events-none text-indigo-500">
                                <Locate size={14} />
                            </div>
                ...
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-1">Ghi chú</label>
                    <div className="relative">
                        <div className="absolute left-2.5 top-2 pointer-events-none text-indigo-500">
                            <Notebook size={14} />
                        </div>
                        <textarea 
                            className="w-full pl-8 pr-3 py-1.5 bg-indigo-900/30 border border-indigo-800 rounded text-xs text-white placeholder-indigo-700 focus:border-yellow-500 outline-none transition-all h-20 resize-none"
