...
import { Bus, Calendar as CalendarIcon, LayoutDashboard, Settings, LogOut, Ticket, Menu, MapPin } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  const getLunarDate = (date: Date) => {
    const lunarDay = date.getDate() > 15 ? date.getDate() - 15 : date.getDate() + 15;
    const lunarMonth = date.getDate() > 15 ? date.getMonth() + 1 : date.getMonth();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex overflow-hidden">
      ...
      {/* Sidebar */}
      <aside 
        className={`bg-white border-r border-slate-200 flex-col fixed h-full z-20 transition-all duration-300 ${isSidebarOpen ? 'w-64 translate-x-0' : 'w-64 -translate-x-full md:translate-x-0 md:w-0 md:opacity-0 md:overflow-hidden'}`}
      >
