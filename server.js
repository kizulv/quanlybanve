...
const ensureItemForTrip = (booking, trip) => {
  let item = booking.items.find(
    (i) => i.tripId === trip.id || i.tripId === trip._id.toString()
  );
  if (!item) {
    item = {
...
app.post("/api/bookings", async (req, res) => {
  try {
    ...
    await booking.save();
    await logBookingAction(booking._id, "CREATE", `Tạo đơn hàng`, {
...
app.put("/api/bookings/:id", async (req, res) => {
  try {
    ...
    let finalStatus =
      requestedStatus ||
      (totalPaid >= calculatedTotalPrice ? "payment" : "booking");
    const targetSeatStatus =
      finalStatus === "payment"
        ? "sold"
        : finalStatus === "hold"
...
app.post("/api/bookings/transfer", async (req, res) => {
  try {
    ...
    if (!booking || !sourceTrip || !targetTrip) {
      return res
        .status(404)
        .json({ error: "Dữ liệu không tồn tại (Booking/Trip)" });
    }
    ...
    for (const transfer of seatTransfers) {
      ...
      const sourceTicketIndex = sourceItem.tickets.findIndex(
        (t) => t.seatId === sourceSeatId
      );
      if (sourceTicketIndex === -1) continue;

      // Clone đối tượng ticket để di chuyển an toàn
      const sourceTicket = JSON.parse(
        JSON.stringify(sourceItem.tickets[sourceTicketIndex])
      );
      ...
      if (targetOccupant) {
        ...
        const tTicketIndex = tItem.tickets.findIndex(
          (t) => t.seatId === targetSeatId
        );
        ...
        sourceItem.seatIds = sourceItem.seatIds.filter(
          (sid) => sid !== sourceSeatId
        );
    ...
    await logBookingAction(
      bookingId,
      "TRANSFER",
      `Điều phối khách sang xe ${targetTrip.licensePlate}`
    );
    ...
    for (const trip of trips) {
      ...
      trip.seats = trip.seats.map((s) => {
        if (
          ...
          if (!occupiedMap.has(`${trip.id}_${s.id}`)) {
            isModified = true;
            fixedCount++;
...
app.post("/api/bookings/swap", async (req, res) => {
  try {
    ...
    if (!booking1)
      return res.status(404).json({ error: "Source has no booking" });
      ...
      if (booking1._id.equals(booking2._id)) {
        booking1.items = booking1.items.map((item) => {
          if (item.tripId === tripId) {
            ...
            let newTickets = item.tickets.map((t) =>
              t.seatId === seatId1
                ? { ...t, seatId: seatId2 }
                : t.seatId === seatId2
                ? { ...t, seatId: seatId1 }
        ...
        const s1s = seat1.status;
        const s2s = seat2.status;
        trip.seats = trip.seats.map((s) =>
          s.id === seatId1
            ? { ...s, status: s2s }
            : s.id === seatId2
